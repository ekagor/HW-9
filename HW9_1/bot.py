...
def export_file(message):
    if message.text == '1':
        ...
        bot.send_document(chat_id, open(r'/phonebook/export.csv', 'rb'))
    elif message.text == '2':
        lg.write_data(f'Запущен экспорт по второму правилу')
        wd.ex_base(message.text)
        bot.send_document(chat_id, open(r'/phonebook/export_2.csv', 'rb'))
    else:
        lg.write_data(f'Ввод неверен: {message.text}')
        bot.send_message(chat_id, 'Вы где то ошиблись')
